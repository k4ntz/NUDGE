...
const std::string NAME = "loothard";
...
class LootHard : public BasicAbstractGame {
  public:
    ...
    void asset_for_type(int type, std::vector<std::string> &names) override {
        if (type == PLAYER) {
            ...
            names.push_back("misc_assets/finish.png");
        }
    }
        ...
        for (int i = 0; i < maze_dim; i++) {
            for (int j = 0; j < maze_dim; j++) {
                ...
                if (obj >= KEY_OBJ) {
                    ...
                    auto ent = spawn_entity(.5 * maze_scale, EXIT, maze_scale * x, maze_scale * y, maze_scale, maze_scale);
                    match_aspect_ratio(ent);
                    exit_door_x = obj_x;
                    exit_door_y = obj_y;
                    // int32_t *data = (int32_t *)(obs_bufs[obs_name_to_offset.at("exit_pos")]);
                    // data[0] = obj_x;
                    // data[1] = obj_y;
                }
    ...
    void observe() override {
        ...
        *(int32_t *)(info_bufs[info_name_to_offset.at("key_count")]) = key_count;
        ...
        int32_t *agdata = (int32_t *)(info_bufs[info_name_to_offset.at("agent_pos")]);
        agdata[0] = agent->x;
        agdata[1] = agent->y;
    }
